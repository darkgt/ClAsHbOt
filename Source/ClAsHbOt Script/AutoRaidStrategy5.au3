;
; Strategy 5 - Barcher, top or bottom
...
Func FillBarracksStrategy5(ByRef $hHMP, Const $initialFillFlag, Const ByRef $builtTroopCounts)
...
Func AutoRaidExecuteRaidStrategy5(ByRef $hBMP)
   ...
   Local $direction = AutoRaidStrategy5GetDirection()
   ...
   If $troopIndex[$eTroopWallBreaker][4] > 0 And _GUICtrlButton_GetCheck($GUI_AutoRaidUseBreakers) = $BST_CHECKED Then
	  ...
	  DeployTroopsToSides($eTroopWallBreaker, $troopIndex, $eAutoRaidDeploySixtyPercent, $direction, $gMaxDeployBoxes)
   EndIf
...
Func AutoRaidStrategy5GetDirection()
   ...
   DebugWrite("AutoRaidStrategy5GetDirection() Collectors found top: " & $collectorsOnTop & ", bottom: " & $collectorsOnBot)

   Local $dir = $collectorsOnTop/($collectorsOnTop+$collectorsOnBot) >= 0.5 ? "Top" : "Bot"

   DebugWrite("AutoRaidStrategy5GetDirection() Attacking from " & $dir)
